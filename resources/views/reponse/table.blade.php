    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                ...
                @else
                    @if (\App\Models\AnneeExercice::getVerifDateDebut($ae,'prevision') == true)

                        @if (\App\Models\AnneeExercice::getDifDate($ae,$reponse->structure_id, 'prevision') >= 0)
                            @if (\App\Models\AnneeExercice::getDifDate($ae,$reponse->structure_id, 'prevision') > 30)
                                <span class="btn btn-sm btn-primary btn-gradient btn-pill m-1">
                                    Il reste {{ \App\Models\AnneeExercice::getDifDate($ae,$reponse->structure_id, 'prevision') }} jour(s)
                                </span>
                            @else
                        ...
                        @else
                            <span class="btn btn-sm btn-danger btn-gradient btn-pill m-1">
                                En retard de {{ abs(\App\Models\AnneeExercice::getDifDate($ae,$reponse->structure_id, 'prevision')) }} jour(s)
                            </span>
                        @endif
            ...
            <td>
                <a href="{!! route('reponses.show', [$reponse->id,'option'=>'prevision']) !!}" id="showList" class="btn btn-info data-tooltip" data-tooltip="Voir la liste">
                ...
                <a href="#" data-url="{!! route('reponses.edit', [$reponse->id,'option'=>'prevision']) !!}" id="modifierList" class="btn btn-info data-tooltip" data-tooltip="Repporter la date">
                    <i class="fa fa-window-restore"></i> Repporter
                </a>
                @else
                @if (\App\Models\Reponse::controleReponse($reponse->id))
                <a href="#" data-url="{!! route('reponses.envoyerListe', [$reponse->id,'option'=>'prevision']) !!}" id="envoyerList" class="btn btn-info data-tooltip" data-tooltip="Repporter la date">
