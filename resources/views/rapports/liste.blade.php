@extends('layouts.app')
@section('titre' , 'Rapports par commune' )
@section('breadcrumb')
        ...
        <div class="card card-primary">
            <div class="card-header bg-light">
                ...
                <div class="row float-left" style="width: 400px;border:2px double blue;color: white">
                    <select class="form-control" name="commune_id" id="id_commune" onChange="getPrevision(this.value);" required  >
                            <option value="">Sélectionnez la commune</option>
                        @foreach(\App\Models\Commune::all() as $commune )
                            <option style="" value="{{$commune->id}}">{{$commune->nom_commune}}</option>
                        @endforeach
                    </select>
                ...
                <div class="row float-left" style="width: 200px;border:2px double blue;margin-left: 2%">
                    <select class="form-control" name="commune_id" id="annee" onChange="getPrevisionAnnee(this.value);" required  >
                ...
                <h1 class="row float-right">
                    ...
                    <a href="{!! route('pdf-communes') !!}"
                       data-url="{!! route('pdf-communes') !!}"
                       id="communePdf"
                       target="_blank" class="btn btn-primary float-right ml-4" >
                        <i class="fa fa-file-import"></i> Imprimer</a>
                    <a href="{!! route('pdf-communes-g') !!}"
                       data-url="{!! route('pdf-communes-g') !!}"
                       id="communePdfg"
                       target="_blank" class="btn btn-primary float-right ml-4" >
...
@section('scripts')
    <script type ="text/javascript">
        function getPrevision(val) {
            ...
            $.ajax({
                type: "GET",
                url: "{{ route('rapports.filtre-communes') }}",
                data:{id_commune :val,annee: annee},
        ...
        function getPrevisionAnnee(val) {
            var commune = document.getElementById('id_commune').value;
            $.ajax({
                type: "GET",
                url: "{{ route('rapports.filtre-communes') }}",
                data:{id_commune :commune,annee: val},
                success: function(data){
                    //$("#id_body").html(data);
                    $('#rapports-table').html(data);
                    //clientSideDataTable.destroy();
        ...
        $(document).on('click', '#communePdf', function (e) {
            e.preventDefault();
            var url = $(this).attr('data-url');
            $.get(url+'?idCommune='+document.getElementById('id_commune').value+'&annee='+document.getElementById('annee').value)
                .done(function (data) {
                    document.location.href = url+'?idCommune='+document.getElementById('id_commune').value+'&annee='+document.getElementById('annee').value;
                })
        });

        $(document).on('click', '#communePdfg', function (e) {
            e.preventDefault();
            var url = $(this).attr('data-url');
